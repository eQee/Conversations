...
import de.eqee.pn.xmpp.OnAdvancedStreamFeaturesLoaded;
...
public class MessageArchiveService implements OnAdvancedStreamFeaturesLoaded {
	...
	private void catchup(final Account account) {
		...
		if (mamReference.getTimestamp() == 0) {
			...
			List<Conversation> pn = mXmppConnectionService.getPn();
	...
	private void finalizeQuery(Query query, boolean done) {
		...
		if (conversation != null) {
			...
			for (Conversation tmp : this.mXmppConnectionService.getPn()) {
