...
public class ExportLogsService extends Service {
	...
	private static final String DIRECTORY_STRING_FORMAT = FileBackend.getPnLogsDirectory() + "/logs/%s";
	...
	private void export() {
		List<Conversation> pn = mDatabaseBackend.getPn(Conversation.STATUS_AVAILABLE);
		pn.addAll(mDatabaseBackend.getPn(Conversation.STATUS_ARCHIVED));
		...
		for (Conversation conversation : pn) {
			...
			progress++;
			mBuilder.setProgress(pn.size(), progress, false);
