...
import de.eqee.pn.crypto.axolotl.OnMessageCreatedCallback;
...
import de.eqee.pn.services.AbstractConnectionManager;
...
import de.eqee.pn.xmpp.jingle.stanzas.Content;
import de.eqee.pn.xmpp.jingle.stanzas.JinglePacket;
import de.eqee.pn.xmpp.jingle.stanzas.Reason;
	...
	final OnFileTransmissionStatusChanged onFileTransmissionStatusChanged = new OnFileTransmissionStatusChanged() {
		...
		public void onFileTransmitted(DownloadableFile file) {
			if (responding()) {
				...
				if (ftVersion == Content.Version.FT_5) { //older Pn will break when receiving a session-info
					sendHash();
				}
				if (message.getEncryption() == Message.ENCRYPTION_PGP) {
