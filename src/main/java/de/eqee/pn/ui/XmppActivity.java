...
import de.eqee.pn.databinding.DialogQuickeditBinding;
...
import de.eqee.pn.entities.Presences;
import de.eqee.pn.services.AvatarService;
import de.eqee.pn.services.BarcodeProvider;
import de.eqee.pn.services.XmppConnectionService;
import de.eqee.pn.services.XmppConnectionService.XmppConnectionBinder;
...
public abstract class XmppActivity extends ActionBarActivity {
	...
	private void switchToConversation(Conversation conversation, String text, boolean asQuote, String nick, boolean pm, boolean newTask) {
		Intent intent = new Intent(this, PnActivity.class);
		intent.setAction(PnActivity.ACTION_VIEW_CONVERSATION);
		intent.putExtra(PnActivity.EXTRA_CONVERSATION, conversation.getUuid());
		if (text != null) {
			intent.putExtra(PnActivity.EXTRA_TEXT, text);
			if (asQuote) {
				intent.putExtra(PnActivity.EXTRA_AS_QUOTE, asQuote);
			}
		}
		if (nick != null) {
			intent.putExtra(PnActivity.EXTRA_NICK, nick);
			intent.putExtra(PnActivity.EXTRA_IS_PRIVATE_MESSAGE, pm);
		}
		if (newTask) {
